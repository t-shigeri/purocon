...
export default function SkinTypeChecker() {
  ...
  const question3 = {
    ...
    options: [
      ...
      { label: '詰まり毛穴', img: '/509f30ca-764e-44d2-8862-649cd982a9bd.png' },
  ...
  const renderQuestion = (question, selected, setSelected, name) => {
    // 画像付き選択肢かどうかをチェック
    const hasImages = question.options.some(option => 
      typeof option === 'object' && option.img
    );
              ...
              {question.options.map((option, index) => {
                ...
                return (
                  <label key={index} className="image-option">
                    ...
                    />
                    <p style={{ display: 'block', margin: 0 }}>{label}</p>
                    {img && (
                      <img
                        src={img}
                        alt={`${label}の参考画像`}
              ...
              return (
                <label key={index} style={{ display: 'block', marginBottom: '8px' }}>
                  <input
                    ...
                    onChange={() => setSelected(label)}
                  />
                  <p style={{ display: 'inline', margin: 0 }}>{label}</p>
                </label>
              );
            })
          )}
  ...
  return (
    <div>
      ...
      <p><b>【1問目】</b></p>
      <p>{question1.title}</p>
      <p>あなたの回答: {selected1}</p>
      
      <p><b>【2問目】</b></p>
      <p>{question2.title}</p>
      <p>あなたの回答: {selected2}</p>
      
      <p><b>【3問目】</b></p>
      <p>{question3.title}</p>
      <p>あなたの回答: {selected3}</p>
